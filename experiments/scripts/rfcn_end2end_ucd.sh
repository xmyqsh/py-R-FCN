...
# ./experiments/scripts/rfcn_end2end_ohem_ucd.sh 0 ResNet-50 ucar_driveless \
#   nop \
#   --set EXP_DIR rfcn_ucd RNG_SEED 42 TRAIN.SCALES "[400,500,600,700]"
...
LOG="experiments/logs/rfcn_end2end_ucd_${NET}_${EXTRA_ARGS_SLUG}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
...
time ./tools/train_net.py --gpu ${GPU_ID} \
  ...
  --imdb ${TRAIN_IMDB} \
  --iters ${ITERS} \
  --cfg experiments/cfgs/rfcn_end2end_ucd.yml \
  ${EXTRA_ARGS}
